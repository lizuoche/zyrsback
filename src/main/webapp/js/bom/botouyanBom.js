						...
						if (templateList.length > 0) {
							...
							for (var i = 0; i < templateList.length; i++) {
								...
								tabhtml += "<tr>"
										...
										+ "</td>"
										+ "<td>"
										+ getInterceptedStr(template.big_image, 20)
										+ "</td>"
										+ "<td>"
										+ getInterceptedStr(template.normal_image, 20)
										+ "</td>"
										+ "<td>"
...
function add(enName) {
	layer.open({
		...
		content : '../action/addBom.jsp?enName=' + enName
	});
}
...
function update(id) {
	layer.open({
		...
		title : "修改驳头眼面料",
		shadeClose : false, // 点击遮罩关闭
		content : '../action/updateBom.jsp?id=' + id
	});
}

//面料生成品牌JSON
function botouyanBomToJson(type){
...
//驳头眼面料品牌详情生成JSON文件
function botouyanDetailToJson(type) {
	// 查询面料品牌详情
	$.ajax({
		type : "post",
